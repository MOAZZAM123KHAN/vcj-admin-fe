  ...
  const handleEditSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const payload: IProduct = {
      ...
      tags: typeof editData.tags === 'string'
        ? (editData.tags as string).split(',').map(t => t.trim()).filter(Boolean)
        : editData.tags
    };
            ...
            <form className="space-y-4 mt-4" onSubmit={handleEditSubmit}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="edit-stock" type="number" placeholder="1" min="0" value={editData.stock} onChange={handleEditChange} name="stock" />
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="edit-priority">Admin Priority</Label>
                  <Input id="edit-priority" type="number" placeholder="0" value={editData.priority} onChange={handleEditChange} name="priority" />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="edit-expiryDate">Offer Expiry Date</Label>
                  <Input id="edit-expiryDate" type="date" value={editData.expiryDate ? new Date(editData.expiryDate).toISOString().split('T')[0] : ""} onChange={handleEditChange} name="expiryDate" />
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="edit-tags">Tags (comma separated)</Label>
                <Input id="edit-tags" placeholder="curated_exclusive, hidden_gem" value={editData.tags} onChange={handleEditChange} name="tags" />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={editData.category} onValueChange={(value) => setEditData((prev) => (prev ? { ...prev, category: value } : prev))}>
                    ...
                    <SelectContent>
                      {FILTERS.category.filter(c => c.value !== 'all').map((category) => (
                ...
                <div className="space-y-2">
                  ...
                  <Select value={editData.material} onValueChange={(value) => setEditData((prev) => (prev ? { ...prev, material: value } : prev))}>
                    <SelectTrigger id="edit-material">
                      <SelectValue placeholder="Select material" />
                    </SelectTrigger>
                    <SelectContent>
                      {FILTERS.material.filter(m => m.value !== 'all').map((material) => (
                ...
                <div className="space-y-2">
                  ...
                  <Select value={editData.gender} onValueChange={(value) => setEditData((prev) => (prev ? { ...prev, gender: value } : prev))}>
                    <SelectTrigger id="edit-gender">
                      <SelectValue placeholder="Select gender" />
                    </SelectTrigger>
                    <SelectContent>
                      {FILTERS.gender.filter(g => g.value !== 'all').map((gender) => (
              ...
              <div className="space-y-2">
                ...
                <Select value={editData.occasion} onValueChange={(value) => setEditData((prev) => (prev ? { ...prev, occasion: value } : prev))}>
                  <SelectTrigger id="edit-occasion">
                    <SelectValue placeholder="Select occasion" />
                  </SelectTrigger>
                  <SelectContent>
                    {FILTERS.occasion.filter(o => o.value !== 'all').map((occasion) => (
              ...
              return (
                <tr key={index} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-900">
                    <div className="text-sm font-medium">{product.boughtQuantity} sold</div>
                    <div className={`text-xs ${product.stock === 0 ? 'text-rose-500 font-bold' : 'text-gray-500'}`}>
